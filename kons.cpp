...
long cmpmem( char* feld, const char* search, int len_feld) 
{
  long i=0;
  int j=-1;
...
int untersuser(const string& uname,__uid_t *uidp/*=0*/, __gid_t *gidp/*=0*/,vector<gid_t> *gids/*=0*/,struct passwd* ustr/*=0*/)
{
	...
	if ((buf = (char*)malloc(bufsize))) {
		getpwnam_r(uname.c_str(), &pwd, buf, bufsize, &result);
		if (result) {
...
void haupt::zeigversion(const string& ltiffv/*=nix*/)
{
	...
	cout<<Txk[T_Quelle]<<blau<<defvors<<meinname<<schwarz<<endl;
...
{
	svec qrueck;
	if (!systemrueck("find /usr -maxdepth 1 -type f -name 'libtiff*'",obverb,oblog,&qrueck,/*obsudc=*/0) &&
			!systemrueck("find /usr/local -maxdepth 1 -type f -name 'libtiff*'",obverb,oblog,&qrueck,/*obsudc=*/0)) {
		tuloeschen("/usr/local/sclibtiff","",obverb,oblog);
